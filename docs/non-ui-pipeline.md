...
arXiv announces Sunday–Thursday at 20:00 America/New_York, subject to holidays/delays. Every run first publishes using cached affiliations, then automatically enriches the published recommendations, followed papers and HF papers with a 900-second maximum extraction budget and republishes the same issue. Recommendations have priority. This includes early retries and watchdog dispatches; the legacy `quick` input remains accepted but no longer skips post-publication enrichment. The 00:10 run also enriches the remaining candidate pool. GitHub handles DST. Open visible pages revalidate every minute and on focus. GitHub scheduling is best effort, not an exact-time delivery guarantee.

The metadata-only refresh does not fetch new listings or change recommendation IDs, order, ranking, batch ID or feedback run ID. It updates `updated_at`, rebuilds institutional follows, persists candidate/affiliation caches, and records the enriched delivery only after the second Pages deployment succeeds. The original delivery is recorded before enrichment, so a later failure cannot lose the exposure ledger. Published affiliations also survive eviction of the Actions cache. `pipeline_status.recommendation_affiliations_resolved` and `recommendation_affiliations_total` report coverage for the selected papers; zero coverage emits an Actions warning. Unknown institutions remain unresolved. Email uses the final deployed snapshot.

For SiliconFlow, affiliation extraction defaults to `Qwen/Qwen3-8B` with `enable_thinking=false`, independently of the long-form analysis model. Other providers retain `MODEL_NAME`; `AFFILIATION_MODEL_NAME` can override either default. Each LLM request has a 60-second read timeout, with at most 50 LLM extractions per refresh (overridable via `AFFILIATION_MAX_LLM_PAPERS`) within the shared time budget. Papers deferred by this limit remain pending and retry on the next run rather than entering the one-hour failure cache. Failure-cache version 4 retries legacy failures after the timeout/limit and template fixes. Deterministic extraction resolves ICML's two-argument affiliation labels, French/Portuguese institution names and known institution abbreviations, ignores commented template examples, and splits TeX-spaced institutions without emitting a second concatenated institution. An email domain alone is not treated as affiliation evidence.
